...
graph LR
    ...
    D --> E[Nmea450Decoder шаблоны]
    E --> F[NmeaDecoder шаблоны]
    F --> G[AisCore побитовый сдвиг]
    F --> H[GeoParser UTM гео-математика]
...
- **`compile_time_pipeline.h`** — Шаблонные интерфейсы классов `NmeaDecoder` и `Nmea450Decoder` для статического сквозного связывания слоев.
...
- **`geo_parser.h` / `geo_parser.cpp`** — Высокоскоростное ядро прикладной математики. Агрегирует разбор 32 типов сообщений (включая *Fugro, Trimble, Leica, Furuno*) и обратную проекцию Гаусса-Крюгера/UTM.
